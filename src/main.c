...
struct ImageConfig
{
	...
	unsigned short CHARCTOR_SIZE_X;
	unsigned short CHARCTOR_SIZE_Y;  
...
struct ImageConfig PL1S=
{
	0x00000000,256,256,2,32,32,
	&Player1_S[0],
	{ 0, 0,NULL },
};
struct ImageConfig PL1N=
{
	0x00000000,256,256,2,32,32,
	&Player1_N[0],
	{ 0, 0,NULL },
};
struct ImageConfig PL1W=
{
	0x00000000,256,256,2,32,32,
	&Player1_W[0],
	{ 0, 0,NULL },
};
struct ImageConfig PL1E=
{
	0x00000000,256,256,2,32,32,
	&Player1_E[0],
	{ 0, 0,NULL },
...
//-------------------------
		if( PL1S.Now.Time == 0 )
		{
			if( PL1S.Now.next != NULL )
			{
				PL1S.Now = *PL1S.Now.next;
			}
		}
		...
		HostMemWriteDoubleWord( RAM_CMD + cmdOffset,BITMAP_LAYOUT(ARGB1555, PL1S.IMAGE_SIZE_X * PL1S.COLOR_DPTH, PL1S.IMAGE_SIZE_Y) );
		cmdOffset = incCMDOffset(cmdOffset, 4);
		HostMemWriteDoubleWord( RAM_CMD + cmdOffset,BITMAP_SIZE(NEAREST, 0, 0, PL1S.CHARCTOR_SIZE_X, PL1S.CHARCTOR_SIZE_Y) );
		cmdOffset = incCMDOffset(cmdOffset, 4);
//-------------------------
		HostMemWriteDoubleWord( RAM_CMD + cmdOffset,BEGIN_BITMAPS );
		cmdOffset = incCMDOffset(cmdOffset, 4);

		HostMemWriteDoubleWord( RAM_CMD + cmdOffset,BITMAP_SOURCE( RAM_G + PL1S.ImageBase + PL1S.Now.ImageOffset ) );
		cmdOffset = incCMDOffset(cmdOffset, 4);

//-------------------------
//		HostMemWriteDoubleWord( RAM_CMD + cmdOffset,VERTEX2F( 100,100 ) );
//		cmdOffset = incCMDOffset(cmdOffset, 4);
		...
		for( y=0;y<GRID_Y;y++ )
		{
			for( x=0; x<GRID_X; x++ )
			{
				HostMemWriteDoubleWord( RAM_CMD + cmdOffset,VERTEX2F( x * 32 ,-16+y * 32 ) );
				cmdOffset = incCMDOffset(cmdOffset, 4);
			}
		}
		...
		cmdOffset = incCMDOffset(cmdOffset, 4);
//-------------------------
		if( PL1S.Now.Time )PL1S.Now.Time--;
//-------------------------
		HostMemWriteDoubleWord( RAM_CMD + cmdOffset,(DL_DISPLAY) );
		cmdOffset = incCMDOffset(cmdOffset, 4);
		HostMemWriteDoubleWord( RAM_CMD + cmdOffset,(CMD_SWAP) );
